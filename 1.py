...
#原始数据一个, 训练数据再加2个假的.
                ...
                if rnd <= 0.5:#我们有一半的概率创造假的triple头
                    # corrupting head
                    tmp_head = ''
                    while True:
                        ...
                        tmp_head = random.choice(tmp_ent_list)#随机抽取一个假head
                        tmp_triple_str = tmp_head + '\t' + line[1] + '\t' + line[2]
                        if tmp_triple_str not in lines_str_set:
                            break                    
                    tmp_head_text = ent2text[tmp_head] #进行去下划线操作.
                    examples.append(
                        InputExample(guid=guid, text_a=tmp_head_text, text_b=text_b, text_c = text_c, label="0"))       
                else:
                    # corrupting tail#另一半概率我们创造假的tail
                    tmp_tail = ''
                    while True:
                        tmp_ent_list = set(entities)
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, print_info = True):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        tokens_a = tokenizer.tokenize(example.text_a)#编码head
        ...
        if example.text_b and example.text_c:
            ...
            #_truncate_seq_pair(tokens_a, tokens_b, max_seq_length - 3) 因为需要4个特殊字符,所以我们最大长度-4
        ...
        segment_ids = [0] * len(tokens) # segment_id表示0,1交替.
...
def _truncate_seq_triple(tokens_a, tokens_b, tokens_c, max_length):
    ...
    while True:
        ...
        if total_length <= max_length:
            break
        if len(tokens_a) > len(tokens_b) and len(tokens_a) > len(tokens_c):#超过长度的就pop到合适位置.
            tokens_a.pop()
...
#使用tensordataset来弄,这样不用建立dataset类别了.更方便.适合这种小数据集.
